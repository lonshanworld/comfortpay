...
const calculateCommission = (transaction: Order, merchant: Merchant) => {
    // The commissionRates object from the database is a JSON string
    ...
    const rate = commissionRates?.[processor] || 0;
    return transaction.totalAmount * (rate / 100);
};
...
export async function GET(
  ...
  try {
    ...
    const commissionStatement = merchants.map(merchant => {
        ...
        const stripeRate = typeof merchant.commissionRates === 'string' ? JSON.parse(merchant.commissionRates)?.stripe : merchant.commissionRates?.stripe;
